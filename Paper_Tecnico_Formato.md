...
Los sistemas fotovoltaicos pueden clasificarse en dos categorías principales según su configuración: sistemas conectados a la red eléctrica (grid-tied) y sistemas aislados con almacenamiento en baterías (off-grid o híbridos). En sistemas conectados a la red, los paneles generan energía que se inyecta directamente a la red eléctrica o se consume en tiempo real, permitiendo que el exceso de generación se venda a la red mediante mecanismos de medición neta (net metering). En sistemas con baterías, los paneles cargan baterías durante el día y la carga se alimenta desde las baterías, proporcionando autonomía energética pero requiriendo dimensionamiento adicional del sistema de almacenamiento. Este trabajo se enfoca en sistemas conectados a la red, que representan la mayoría de las instalaciones residenciales y comerciales en áreas urbanas con acceso a la red eléctrica.

Los métodos tradicionales de dimensionamiento fotovoltaico a menudo se basan en reglas empíricas simples que no consideran la variabilidad climática local, calculadoras básicas que usan promedios imprecisos, o software especializado que requiere expertise técnico y es costoso. Este trabajo propone una solución alternativa mediante el desarrollo de un simulador web interactivo que utiliza un modelo de regresión basado en datos reales de generación fotovoltaica para sistemas conectados a la red.
...
La comparación con benchmarks de la industria muestra que nuestro modelo con R² de 0,970 y RMSE de 5,8 W presenta métricas superiores a herramientas reconocidas. Según estudios de validación, PVWatts de NREL reporta una precisión típica del 4-7% para sistemas residenciales en Estados Unidos [2], mientras que PVGIS, herramienta similar a SolarGIS basada en datos satelitales, presenta una precisión del 3-5% para instalaciones en Europa [6]. SolarGIS, por su parte, ha demostrado un error cuadrático medio normalizado (nRMSE) de 14,09% en la estimación de irradiancia global horizontal [7]. Nuestro modelo, con un MAPE de 3,5% y RMSE de 5,8 W, muestra un rendimiento comparable o superior a estas herramientas establecidas, especialmente considerando que está validado específicamente para las condiciones del dataset utilizado.
...
Las limitaciones del simulador incluyen que requiere que el usuario proporcione datos de PSH y temperatura mensual manualmente, aunque se pueden obtener mediante integración con APIs de clima externas como NASA POWER que se ejecutan desde el frontend, no considera diferentes tipos de paneles en el cálculo base, no calcula configuración óptima de conexión serie-paralelo y no dimensiona inversores. El simulador está diseñado específicamente para sistemas conectados a la red eléctrica (grid-tied), por lo que no considera sistemas de almacenamiento en baterías, los cuales requerirían modelado adicional de ciclos de carga-descarga, profundidad de descarga, eficiencia del sistema de almacenamiento y gestión de energía. Estas limitaciones son inherentes a la simplicidad de la arquitectura frontend-only y al enfoque en sistemas conectados a la red, pero permiten mantener la aplicación ligera y accesible para el caso de uso más común.

4.3. Comparación con Alternativas

La comparación con alternativas muestra que nuestro simulador presenta métricas de precisión (R² de 0,97, MAPE de 3,5%) comparables o superiores a herramientas establecidas como PVWatts (precisión típica 4-7% [2]) y PVGIS/SolarGIS (precisión 3-5% [6]). Todas las herramientas utilizan datos reales para validación. Nuestro simulador ofrece ventajas distintivas: es completamente gratuito sin requerir registro, código open source disponible públicamente, incluye análisis financiero completo integrado y funciona completamente en frontend sin dependencias de servidor, características que no comparten todas las alternativas comerciales.
...
[6]	PVGIS, "PVGIS vs PVWatts: Accuracy Comparison", *European Commission Joint Research Centre*, 2023. Disponible en: https://pvgis.com/en/blog/pvgis-vs-pvwatts-accuracy

[7]	Solcast, "Historical and TMY: SolarGIS Validation", *Solcast*, 2023. Disponible en: https://solcast.com/historical-and-tmy/solargis

[8]	Sergio, "Mínimos Cuadrados y Energía Solar", *Universidad de Sevilla*, s.f. Disponible en: https://personal.us.es/sergio/PDocente/lectura.pdf

[9]	"Clase: Cuadrados Mínimos", *Universidad de Buenos Aires - Departamento de Física*, 2012. Disponible en: https://materias.df.uba.ar/l1b2022c2/files/2012/07/CLASE-Cuadrados-Minimos.pdf

[10]	PV Magazine Latinoamérica, "Algoritmos recursivos de mínimos cuadrados para detectar el funcionamiento anómalo de los sistemas fotovoltaicos", *PV Magazine Latinoamérica*, Junio 2024. Disponible en: https://www.pv-magazine-latam.com/2024/06/27/algoritmos-recursivos-de-minimos-cuadrados-para-detectar-el-funcionamiento-anomalo-de-los-sistemas-fotovoltaicos/

[11]	García Hombrados, G., "Dimensionamiento de Sistemas Fotovoltaicos mediante Simulación Web", *Trabajo de Fin de Grado, Universidad Miguel Hernández*, s.f. Disponible en: https://dspace.umh.es/bitstream/11000/29079/1/TFG-Garc%C3%ADa%20Hombrados%2C%20Gonz%C3%A1lo.pdf

[12]	SFE Solar, "Parámetros de Paneles Solares", *SFE Solar*, s.f. Disponible en: https://www.sfe-solar.com/paneles-solares/parametros/

[13]	Efergia, "Energía Solar Fotovoltaica", *Efergia*, s.f. Disponible en: https://efergia.com/

[14]	Solfy, "¿Cuántos paneles solares se necesitan para dar energía a una casa?", *Solfy*, s.f. Disponible en: https://solfy.net/autoconsumo/placas-solares/cuantos-paneles-solares-se-necesitan-para-dar-energia-a-una-casa/

[15]	"Dimensionamiento de Sistemas Fotovoltaicos", *YouTube*, s.f. Disponible en: https://www.youtube.com/watch?v=Zf2Fj11Ll_k
